...
export default function CreateSlotForm({storageID}: {storageID: bigint}) {
    const { t } = useTranslation();
    const jwt = useContext(AuthContext);
    const navigate = useNavigate();
    const [maxWeightState, setMaxWeightState] = useState(0);

    async function createSlot(maxWeight: number) {
        const response = await fetch(import.meta.env.VITE_BACKEND_URL + '/admin/slot', {
            ...
            body: JSON.stringify({"storageID": storageID, "maxWeight": maxWeight})
        });
        if (!response.ok) {
